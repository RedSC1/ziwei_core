...
> 本库底层农历与节气算法以东八区（UTC+8，北京时间）及中国历书为基准。越南、老挝等使用 UTC+7 农历规则的地区，因朔望月日期边界与中国历书存在本质差异（同一朔望月在北京时间为初一、在河内时间可能为腊月三十），**默认参数并不适用于这些地区**。如需支持非中国农历体系，开发者需自行实现类似 `TimeAdapter` 的转换层，而非仅修改 `timeZone` 或 `location` 参数。
