...
- **平太阳时（默认）**：直接使用标准时区时间（北京时间 UTC+8），不做经度修正。大多数现代流派使用这种方式。
- **真太阳时**：根据出生地的精确经度，修正与标准时区之间的时差，有时能改变时辰边界。
...
// 此时 trueSolarTime getter 会返回修正后的时间
// 时辰判断基于修正后的时间
```

> [!TIP]
> 如果不传 `location`，引擎默认使用 `(120E, 30N)` 作为基准点（与 UTC+8 基本重合），相当于不修正，即平太阳时模式。
...
### 1. 立春前出生，四化/大限顺逆与预期不符
年干的界定方式（农历年还是节气年）由 `siHuaBasedOn` 和 `wuHuDunBasedOn` 控制。如果某人生于 1月底（农历年和节气年不一致），请明确选择流派后再设置对应的 `CalendarOptions`。
...
### 4. 公元前生辰无法用 `DateTime`
Dart 原生的 `DateTime` 最早只支持公元元年。如果需要推算历史人物（如古人），必须使用 `AstroDateTime`：
```dart
// 公元前 179 年（汉文帝）
final date = ZiweiDate.fromSolar(
