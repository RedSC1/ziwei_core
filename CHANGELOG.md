...
- **[新功能] Tier 1 紫微盘反查**：通过指定星曜宫位直接反推出生时间，O(1) 无循环。
  ...
  - 使用 `ssq.calcY` 校验大小月，纯数字索引计算，兼容全部历史历法月名（"后九""十三""拾贰"等）。
  - 自动枚举闰月邻居候选（常规月 + 闰上月 + 闰本月）。
...
- **[依赖升级]** 升级 `sxwnl_spa_dart` 至 `^0.18.5`，`bazi_core` 至 `^0.6.7`。
- **[时辰边界修复]** 同步底层八字时辰整点边界修复，解决 `13:00:00` 等整点因 JD 浮点精度漂移导致时辰判定偏差的问题。

## 0.12.10

- **[依赖升级]** 升级 `bazi_core` 至 `^0.6.6`。
- **[能力同步]** 同步底层八字反查 API，支持按年/月/日柱反查日期、按日期候选反查时辰，以及完整八字反查入口。
- **[子时语义对齐]** 对齐底层 `RatHourMode` 反查语义，确保 `noSplit / todayGan / tomorrowGan` 在晚子时场景下与正向排盘保持一致。
