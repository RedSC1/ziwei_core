...
- 将 `ephemeris_lite` 依赖固定到稳定版 `^1.0.0`。
- 完成出生盘、流运管理器、历史时间线、自定义规则、修改盘、起盘和反查的 JS／C++／Dart Web 验证。

## 1.0.0-beta.1

- **[重大破坏性更新]** 基于 `ziwei-lite` 重写公共 API，与旧版 `0.13.x` 不兼容；旧 `TimePack` 时间封装改为显式的物理时刻、计算钟表和逐盘选项，默认采用民用钟表时间。迁移方式见 `doc/migration.md`。
