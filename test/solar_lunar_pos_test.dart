...
void main() {
  ...
  group('eV / mV (角速度)', () {
    test('地球角速度 ≈ 628.33 rad/century', () {
      final v = eV(0);
      // 地球每世纪转约 628.33 弧度 (≈100圈)
      expect(v, closeTo(628.33, 1));
    });

    test('月球角速度 ≈ 8399.7 rad/century', () {
      final v = mV(0);
      // 月球每世纪转约 8400 弧度 (≈1337圈)
      expect(v, closeTo(8399.7, 50));
    });
  ...
  group('sALonT / sALonT2 (已知太阳黄经求时间)', () {
    test('春分 (太阳黄经=0) 求时间', () {
      final t = sALonT(0); // 太阳黄经为0的时刻
      // 应该在某个春分附近，距J2000大约79天
      final days = t * 36525;
      // J2000后第一个春分约在第79天
    ...
    test('夏至 (太阳黄经=π/2) 求时间', () {
      final t = sALonT(math.pi / 2); // 太阳黄经为90°
      final days = t * 36525;
      // J2000后第一个夏至约在第172天
