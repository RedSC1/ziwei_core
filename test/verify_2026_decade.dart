...
void main() async {
  ...
  final globalOptions = ruleset.calendarOptions;
  ...
  print(
    '📅 出生时间: ${birthday.year}-${birthday.month}-${birthday.day} ${birthday.hour}:${birthday.minute}',
  );
  print(
    '📋 判定八字: ${ziweiDate.bazi.year.gan.label}${ziweiDate.bazi.year.zhi.label} '
...
void _printPalaceInfo(ZiWeiPlate plate, Palace palace) {
  ...
  final roles = <String>[];
  roles.add(
    "原${_calculateRelativeName(plate.originMingIndex, palace.index, "")}",
  );
