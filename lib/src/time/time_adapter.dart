...
class TimeAdapter {
  static ZiweiDate fromSolar(DateTime date, {CalendarOptions? options}) {
    ...
    if (!opt.splitRatHour && date.hour >= 23) {
      virtualDate = virtualDate.add(const Duration(hours: 1));
    } else {
      virtualDate = date;
    }

    // 1. 调用第三方库计算
    Lunar lunar = Lunar.fromDate(virtualDate);
  ...
  static ZiweiDate fromLunar(
    ...
    Lunar lunarObj = Lunar.fromYmdHms(
      year,
      targetMonth,
      day,
      hourIndex * 2,
