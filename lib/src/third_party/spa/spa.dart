...
class SPAParams {
  ...
  /// Manually specified Julian Day.
  ///
  /// If provided, this will override the JD calculated from [time].
  /// This allows supporting BCE dates or high-precision times that
  /// Dart's DateTime cannot represent.
  double? manualJD;
}
...
Tuple2<double, double> _rightAscentionParallax(
  ...
  return Tuple2(
    ...
    _r2d(
      atan2(
        (sin(deltaRad) - y * sin(xiRad)) * cos(dtAlphaRad),
        cos(deltaRad) - x * sin(xiRad) * cos(hRad),
      ),
    ),
  );
  ...
  return _r2d(
    acos(
      cos(zenithRad) * cos(slopeRad) +
          sin(slopeRad) *
              sin(zenithRad) *
              cos(_d2r(azimuthAstro - azmRotation)),
    ),
  );
}
...
double _sunHourAngleAtRiseSet(double latitude, double dtZero, double h0Prime) {
  ...
  final argument =
      (sin(_d2r(h0Prime)) - sin(latitudeRad) * sin(dtZeroRad)) /
...
double _rtsSunAltitude(double latitude, double deltaPrime, double hPrime) {
  ...
  return _r2d(
    asin(
      sin(latitudeRad) * sin(dtPrimeRad) +
...
void _calculateEotAndSunRiseTransitSet(
  ...
  final mRtsTransit = _approxSunTransitionTime(
    alpha.item2,
    params.longitude,
    nu,
  );
...
SPAOutput spaCalculate(
  ...
  it.jd =
      params.manualJD ??
      _julianDay(params.time, params.second, params.timeZone, params.deltaUtl);
