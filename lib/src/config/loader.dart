...
class ConfigLoader {
  static CalendarOptions parse(String? jsonStr) {
    ...
    return CalendarOptions(splitRatHour: splitRat, leapRule: leapRule);
  }


  static Map<String, dynamic> _requireMap(Map<String, dynamic> map, String key) {
    if (!map.containsKey(key)) throw ArgumentError('❌ Missing field: $key');
    final val = map[key];
    if (val is! Map<String, dynamic>) throw ArgumentError('field "$key" must be Object');
    return val;
  }
  ...
  static LeapMonthRule _parseLeapRule(String str) {
    switch (str) {
      case 'split': return LeapMonthRule.splitAt15;
      case 'current': return LeapMonthRule.asPrevious;
      case 'as_next': return LeapMonthRule.asNext;
      default: throw ArgumentError('❌ Invalid leap strategy: $str');
    }
  }
}